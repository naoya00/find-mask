...
１つ目のポイントはユーザーにストレスを感じないようにUI/UXを考えた点です。<br>
headerに常に、全サービス飛べるようにメニューバーを設置しております。<br>
...
今回実装したインクリメンタルサーチは、出力する情報が別テーブル且つ複数あったのでとても難しく<br>
append先が全て一緒になってしまうので、appendする先をidで変化させると言う工夫をして乗り越えました。<br>
この問題は直接調べても、答えがありませんでした。そのような問題に直面した際は情報を組み合わせて解決する。<br>
